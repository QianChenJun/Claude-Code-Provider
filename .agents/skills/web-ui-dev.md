---
name: web-ui-dev
description: 修改或调试 AI CLI Switcher Web 管理界面
...
# 方式 2: 直接启动
node src/server.mjs --port 15723 --tool claude --auth-token dev123

# 访问授权入口
# http://127.0.0.1:15723/auth?token=dev123&tool=claude
```
...
- ❌ 不要破坏 per-tool 内存机制
- ❌ 不要在前端日志打印 token
- ❌ 不要直接修改安装目录中的 Web 文件，应修改 `src/web/` 源码
...
# 4. 检查配置文件
Get-Content -LiteralPath $env:USERPROFILE\.claude\provider-profiles\providers.json -Raw -Encoding UTF8
...
```powershell
# 读取配置
Invoke-WebRequest -Uri "http://127.0.0.1:15723/api/claude/config" `
