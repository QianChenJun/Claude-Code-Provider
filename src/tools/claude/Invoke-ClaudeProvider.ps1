#!/usr/bin/env pwsh
# Thin wrapper for claude provider launcher.
# Called by generated bin shims: & 'E:\Desktop\Claude-Provider-Profiles\src\tools\claude\Invoke-ClaudeProvider.ps1' @args
$ErrorActionPreference = 'Stop'

. (Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'src\tools\Import-Core.ps1')
Import-ProviderCore

$tool = Get-ProviderTool -Name 'claude'
...
Invoke-ProviderSession -ToolName 'claude' -ProfileId $profileId -RemainingArgs $remaining