...
const syncScript = path.join(root, 'Sync-ClaudeProfileShortcuts.ps1');
const commandPrefix = 'ccp';
...
const port = portIndex >= 0 ? Number(args[portIndex + 1]) : 15723;
...
function normalizeProfile(input) {
  ...
  if (input.extraEnv && typeof input.extraEnv === 'object' && !Array.isArray(input.extraEnv)) {
    profile.extraEnv = {};
    for (const [key, value] of Object.entries(input.extraEnv)) {
      if (key.trim()) {
        profile.extraEnv[key.trim()] = String(value);
      }
    }
  }
  return profile;
...
function validateProfile(profile) {
  ...
  if (!['ANTHROPIC_AUTH_TOKEN', 'ANTHROPIC_API_KEY'].includes(profile.authEnv)) {
    return 'authEnv 只能是 ANTHROPIC_AUTH_TOKEN 或 ANTHROPIC_API_KEY';
  }
  return null;
}
  ...
  try {
    ...
    if (req.method === 'PUT' && url.pathname === '/api/config') {
      ...
      for (const [id, value] of Object.entries(incoming.profiles)) {
        ...
        const commandNames = [profile.shortcut || `${id}-claude`, `${commandPrefix}-${id}`];
...
server.listen(port, '127.0.0.1', () => {
  console.log(`Claude 供应商配置管理： http://127.0.0.1:${port}/`);
