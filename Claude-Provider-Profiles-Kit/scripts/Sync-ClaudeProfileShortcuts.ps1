...
$launcherPath = Join-Path $root 'Invoke-ProviderClaude.ps1'
...
foreach ($name in @($commandPrefix, "$commandPrefix-list", "$commandPrefix-sync", "$commandPrefix-manager", 'provider-claude', 'claude-profile-manager', 'sync-claude-profiles')) {
