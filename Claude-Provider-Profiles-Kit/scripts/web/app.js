...
function collectProfileFromCard(card) {
  ...
  const extraEnvText = getField(card, 'extraEnv').value.trim();
  if (extraEnvText) {
    const extraEnv = JSON.parse(extraEnvText);
    if (!extraEnv || typeof extraEnv !== 'object' || Array.isArray(extraEnv)) {
      throw new Error(`${getField(card, 'id').value.trim()}: extraEnv 必须是 JSON 对象`);
    }
    profile.extraEnv = extraEnv;
  }
  return profile;
}
...
function duplicateProfile(card) {
  ...
  profile.shortcut = `${nextId}-claude`;
...
function removeProfile(card) {
  ...
  const displayName = getField(card, 'displayName').value.trim() || id || '未命名供应商';
...
function renderProfile(id, profile = {}, options = {}) {
  ...
  getField(node, 'authEnv').value = profile.authEnv || 'ANTHROPIC_AUTH_TOKEN';
  getField(node, 'extraEnv').value = profile.extraEnv ? JSON.stringify(profile.extraEnv, null, 2) : '';
...
function updatePreview(card) {
  ...
  const shortcut = getField(card, 'shortcut').value.trim() || `${id}-claude`;
  ...
  card.querySelector('[data-preview]').textContent = `推荐：ccp-${id}；通用：ccp ${id}；兼容旧命令：${shortcut}`;
}
...
function collectConfig() {
  ...
  for (const card of profilesEl.querySelectorAll('.profile-card')) {
    ...
    const profile = collectProfileFromCard(card);
    const commandNames = [profile.shortcut || `${id}-claude`, `ccp-${id}`];
