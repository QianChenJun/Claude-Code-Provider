...
try {
    ...
    Assert-Equal -Actual $result.ApiKeyEnv -Expected 'MI_TEST_CLAUDE_API_KEY' -Message '返回结果应包含默认 apiKeyEnv'
    Assert-Equal -Actual $env:MI_TEST_CLAUDE_API_KEY -Expected 'secret-value' -Message 'Process 环境变量应立即可用'
    ...
    Assert-Equal -Actual $profile.apiKeyEnv -Expected 'MI_TEST_CLAUDE_API_KEY' -Message '应保存 apiKeyEnv'
    Assert-Equal -Actual $profile.model -Expected 'test-model' -Message '应保存模型'
    Assert-True -Condition (-not ($profile.PSObject.Properties.Name -contains 'apiKey')) -Message '不应把明文 API Key 写入配置文件'
