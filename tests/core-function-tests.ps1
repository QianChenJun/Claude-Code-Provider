...
'@
    ...
    # shortcut 与 prefix-id / 配置 ID 同名时去重后应可同步；并生成裸配置 ID 命令
...
'@
    ...
    Assert-True -Condition (Test-Path -LiteralPath (Join-Path $codexBin 'temp.ps1')) -Message '应生成裸配置 ID 命令 temp.ps1'
    Assert-True -Condition (Test-Path -LiteralPath (Join-Path $codexBin 'cdp-any.ps1')) -Message 'shortcut=prefix-id 时应生成 cdp-any.ps1'
    Assert-True -Condition (Test-Path -LiteralPath (Join-Path $codexBin 'any.ps1')) -Message '应生成裸配置 ID 命令 any.ps1'
